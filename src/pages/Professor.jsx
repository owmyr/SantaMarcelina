...
import { getAlunosByTurma, getConfig, getResposta, upsertResposta, classifyValor } from '../lib/storage'
import { exportToCSV, exportToXLSX } from '../lib/csv'

const CAMPOS = {
  desempenho: [
    { key: 'aproveitamento', label: 'Aproveitamento da disciplina', hint: 'Como está o rendimento geral?' },
    { key: 'participacao', label: 'Participação em sala', hint: 'Participa, faz perguntas?' },
    { key: 'cumprimento', label: 'Cumprimento dos prazos', hint: 'Entrega atividades em dia?' },
    { key: 'progresso', label: 'Progresso em relação a si mesmo', hint: 'Evoluiu vs. início do tri?' },
  ...
  pedagogico: [
    ...
    { key: 'respostasPositivas', label: 'Respostas positivas às intervenções', hint: 'Respondeu bem ao apoio?' },
  ],
  comportamentoAcoes: [
...
const OPCOES_SEGMENTED = [
  ...
  { value: 'NÃO', label: 'Não', desc: 'Atenção necessária', cls: 'bg-red-500 text-white border-red-500 shadow' },
]
...
function normalizeSegmentValue(v){
  ...
  if(c==='nao') return 'NÃO'
  // custom legacy like "CONVERSA" -> keep as is but map to PARCIAL visually? For desempenho, treat custom as PARCIAL
...
function SegmentedField({ label, hint, value, onChange }) {
  ...
  // if legacy custom like BAIXO, norm will be NÃO, but we want to show legacy tooltip
    ...
    <div className={`p-3 rounded-xl border-2 transition ${norm ? 'border-sky-200 bg-sky-50/50' : 'border-slate-200 bg-white'}`}>
      <div className="flex items-start justify-between gap-2 mb-2.5">
        <div>
          ...
          {isCustomLegacy && <div className="text-[11px] text-amber-700 mt-1">Valor legado: <span className="font-mono bg-amber-100 px-1 rounded">{displayValue}</span> → será salvo como {norm}</div>}
        </div>
        ...
        {OPCOES_SEGMENTED.map(op=>{
          ...
          return (
            <button
              ...
              className={`px-2 py-2 rounded-xl border-2 text-xs font-semibold transition flex flex-col items-center gap-0.5 ${active ? op.cls : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300 hover:bg-slate-50'}`}
            >
              <span>{op.label}</span>
              <span className={`text-[10px] leading-none ${active ? 'text-white/90' : 'text-slate-400'}`}>{op.desc}</span>
            </button>
          )
        })}
      </div>
...
export default function Professor() {
  ...
  const update = (key, val)=>{
    ...
    // autosave
    if(aluno) {
      upsertResposta({ turma, componente, trimestre, alunoNumero: aluno.numero, alunoNome: aluno.nome, dados: nd })
      setSavedAt(new Date())
    }
  }
